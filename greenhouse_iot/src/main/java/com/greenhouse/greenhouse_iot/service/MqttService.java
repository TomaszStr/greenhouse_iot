...
import com.greenhouse.greenhouse_iot.model.mqtt.*;
...
public class MqttService {
    ...
    public void publishMessage(String topic, String payload, Integer qos) throws MqttException {
    ...
    public MqttCredentials configureMqttForNewSensor(Sensor sensor){
        ...
        CreateClientCommand createClientCommand = new CreateClientCommand(mqttClientId, sensorMqttName, roleName, password, List.of(new String[]{roleName}));
        ...
        try {
            ...
            publishMessage(mqttDynamicSecurityTopic, payload, 0);
    ...
    public MqttCredentials resetMqttPasswordForSensor(Sensor sensor){
        ...
        ModifyClientCommand modifyClientCommand = new ModifyClientCommand(mqttClientId, newPassword);
        CommandsWrapper commandsWrapper = new CommandsWrapper(List.of(new MqttCommand[]{modifyClientCommand}));
        try {
            String payload = objectMapper.writeValueAsString(commandsWrapper);
            publishMessage(mqttDynamicSecurityTopic, payload, 0);

            log.info("Modified mqtt client: {}", mqttClientId);
            return new MqttCredentials(mqttClientId, newPassword);
        } catch (JsonProcessingException e) {
            throw new RuntimeException("Failed to serialize command: " + e.getMessage(), e);
        } catch (MqttException e) {
            throw new RuntimeException("Failed to publish MQTT message: " + e.getMessage(), e);
    ...
    public boolean setSoilMoistureAlertThreshold(String sensorMqttName, Integer threshold) {
        String topic = "devices/" + sensorMqttName + "/commands";
        String payload = String.format("{\"command\": \"setHeight\", \"height\": %d}", threshold);
        try {
            publishMessage(topic, payload, 0);
            return true;
        } catch (MqttException e) {
